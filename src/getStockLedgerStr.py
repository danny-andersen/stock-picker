...
def getAccountSummaryStrs(accountSummary: AccountSummary):
    ...
    summary.appendChild(tr(td("Avg Dividend Yield"), td(f"{mean(accountSummary.dividendYieldByYear.values()) if len(accountSummary.dividendYieldByYear) > 0 else 0:0.2f}%")))
    ...
    summary.appendChild(tr(td("Total Return (Paper gain, divi / income / int paid, realised gain, less fees and costs)"),td(f"£{accountSummary.totalGainLessFees():,.0f} ({accountSummary.totalGainPerc():0.2f}%)")))
    ...
    for yr in [lastTaxYear, currentTaxYear]:
        ...
        txns.extend(list(accountSummary.interestTxnsByYear[yr]) if yr in accountSummary.interestTxnsByYear else list())
