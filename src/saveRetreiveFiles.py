...
def retreiveLocal(fileName):
    ...
    if os.path.exists(fileName):
        ...
        try:
            ...
            print(
                f"!!!!!!!!!!!! Failed to parse json in file {fileName} - ignoring file !!!!!!!"
            )
        fp.close()
    return jsonContent
...
def getAllStockTxnSaved(config):
    ...
    for acc in accounts:
        ...
        for stockFile in stockFiles:
            ...
            txnByStockByAcc[acc][stock] = getStockTxnSaved(
                storeConfig, owner, acc, stock
            )
...
def saveStringToDropbox(config, path, dataStr):
    ...
    if dropBoxRemote:
        ...
        with open(f"{dbDir}{path}", mode="w+t", encoding="utf-8") as fp:
            fp.write(dataStr)


def retrieveStringFromDropbox(config, path):
    dropboxAccessToken = config["dropboxAccessToken"]
    dropBoxRemote = config.getboolean("localStore")
    retStr = "NO FILE READ"
    if dropBoxRemote:
        ...
        retStr = str(res.content, encoding="utf-8")
